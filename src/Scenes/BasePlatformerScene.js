...
// Notes on structure:
//  - Separate files for physics and inits from functionality, particles, and enemy ai handling? (How well would phaser handle this?)
//  - Figure out how to spawn enemies according to phaser physics instead of specifying coords in each level
//  - Coyote time
//  - Acceleration tutorial / Guide for beginners


// Add EnemyState constants at the top of the file (outside the class)
const EnemyState = {
    IDLE: "IDLE",
    PATROL: "PATROL",
    CHASE: "CHASE",
...
class BasePlatformerScene extends Phaser.Scene {
    ...
    createGameObjects(layerName, objectName, frameId) {
        ...
        objects.forEach(obj => {
            ...
            // Initialize enemy AI properties with FSM
            if (objectName === "enemy") {
                obj.state = EnemyState.PATROL;  // Start patrolling by default
                obj.stateTimer = 0;             // Timer for state transitions
                obj.originalX = obj.x;
                obj.originalY = obj.y;
                ...
                obj.hasAttacked = false; // For attack cooldown
            }
        });
    ...
    createEnemiesAtCoordinates(coordinates) {
        ...
        coordinates.forEach(coord => {
            ...
            // Enemy AI properties with FSM
            enemy.state = EnemyState.PATROL;  // Start patrolling by default
            enemy.stateTimer = 0;             // Timer for state transitions
            ...
            enemy.hasAttacked = false; // For attack cooldown
    ...
    // Enemy AI with Finite State Machine
    updateEnemyAI() {
        ...
        this.enemies.children.entries.forEach(enemy => {
            ...
            switch (enemy.state) {
                case EnemyState.IDLE:
                    this.handleIdle(enemy);
                    break;
                case EnemyState.PATROL:
                    this.handlePatrol(enemy);
                    break;
                case EnemyState.CHASE:
                    this.handleChase(enemy);
                    break;
    ...
    handleIdle(enemy) {
        ...
        // transition to patrol after a delay
        enemy.stateTimer++;
        if (enemy.stateTimer > 120) { // ~2 seconds at 60fps
    ...
    handlePatrol(enemy) {
        // Move left/right within patrol bounds
        ...
        // If player enters detection range → chase
        const dist = Phaser.Math.Distance.Between(enemy.x, enemy.y, my.sprite.player.x, my.sprite.player.y);
        if (dist < this.ENEMY_DETECTION_RANGE) {
    ...
    handleChase(enemy) {
        ...
        // Transition: too far → patrol
        const dist = Phaser.Math.Distance.Between(enemy.x, enemy.y, my.sprite.player.x, my.sprite.player.y);
        if (dist > this.ENEMY_FOLLOW_RANGE) {
            enemy.state = EnemyState.PATROL;
        }
    ...
    handleAttack(enemy) {
        ...
        // Damage player here, with cooldown
        if (!enemy.hasAttacked) {
            this.playerHealth -= 10;
            enemy.hasAttacked = true;
            this.time.delayedCall(1000, () => { enemy.hasAttacked = false; });
            
            // Display health when hit
            // Note: change ui when health bar implemented --
            const healthText = this.add.text(
                my.sprite.player.x,
